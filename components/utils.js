...
function createCloseButton(popup) {
  ...
  closeButton.onclick = () => {
    document.getElementById('SignSync-wrapper').remove();

    if (window.signSyncActiveListener) {
      chrome.runtime.onMessage.removeListener(window.signSyncActiveListener);
      window.signSyncActiveListener = null;
    }

    window.signSyncInjected = false;
  };
